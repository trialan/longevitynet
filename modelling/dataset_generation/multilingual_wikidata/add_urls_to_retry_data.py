...
from multilingual_wikidata.utils import retry_if_exception
...
@retry_if_exception(wait_time=4)
...
files = sorted(os.listdir(input_directory), key=lambda x: int(x.split('_')[-1].split('.')[0]))[460:]
...
for i, filename in tqdm(enumerate(files)):
    if filename.endswith(".csv"):
        ...
        for start in range(0, len(entity_ids), batch_size):
            ...
            time.sleep(8)  # Add a small delay between batches to prevent overloading the server
